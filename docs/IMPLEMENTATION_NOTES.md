...
当前 MVP 使用三种固定内置角色：`admin`（管理员）、`operator`（运维）、`viewer`（只读）。这不是完整 RBAC——不支持自定义角色、权限矩阵编辑、菜单级动态权限配置或多租户授权。

- 用户可以有直接角色，也可以通过启用状态的用户组继承角色。后端统一计算 `effective_role`，控制面权限判断以此为准。
- `admin` 可管理用户、用户组、Agent 配置、collector registry、Trash 物理清理、模型、Runtime、实例、审计。
- `operator` 可管理模型、Runtime、实例启停和状态检查，但不能管理用户和关键系统设置。
- `viewer` 只读查看节点、GPU、模型、Runtime、实例、日志和配置摘要。
- 后端已实现最后一个 admin 保护：不能禁用或降级最后一个启用的管理员。当前无用户删除功能。
- 后续如需扩展权限，应结合 API Key、租户、计费和业务归属统一定义权限模型，不要轻易把当前三角色扩展成页面可配置权限矩阵。
...
- **HTTPS**：Server 默认使用 HTTPS 18443（`[https]` 配置）；HTTP 18080 默认关闭，仅可选本机排障（127.0.0.1 only）。
- **证书生成**：`lightai-server cert init` 通过 rcgen（纯 Rust，无 openssl 依赖）生成自签 CA 和 Server 证书。`cert fingerprint` 输出 SHA256 指纹；`cert setup-token` 生成初始化口令。
- **setup token**：空库时 `/api/setup/admin` 必须提供有效的 setup token（fail-closed）。token 不入库、不写日志。
- **Agent TLS**：Agent `[server]` 配置包含 `ca_cert_path` 和 `insecure_skip_tls_verify`。正式通信必须使用 ca.crt 校验 Server 证书。`lightai-agent ca fetch` 下载公开 CA 并显示指纹确认。
- **per-agent token**：Agent 注册成功后使用 Bearer token。Server 只保存 token hash/prefix。当前不自动轮换；如需轮换应禁用/重新注册 Agent。后续可扩展 revoke/rotate。
- **ca.crt 可分发**，ca.key / server.key 不可分发。不要求目标机安装 openssl/python/curl。
- **当前版本无 Agent 安装码**，适合可信内网/客户测试网段；不建议公网暴露 18443。

## 本地部署验证

`scripts/verify-local-deployment.sh` 设计原则：

- **固定 workdir**：默认 `.local/lightai-deployment`（可通过 `LIGHTAI_VERIFY_DIR` 或 `--workdir` 覆盖），避免污染源码目录。
- **智能默认模式**：自动检测 workdir 是否已初始化，决定 fresh 或 update。
- **fresh 模式**：完整重建 certs/配置/数据库/collector sync。
- **update 模式**：只同步无状态目录（bin/web/dist/collectors/scripts），保留 certs/data/logs/run/toml。
- 服务默认保持运行。`--stop` 只停止服务；`--stop-after-verify` 验证后停止。`--clean` 停止服务后删除部署目录。
- **安全性**：`--fresh` 删除前要求确认（或 `--yes`），会先备份旧目录。

## Release 包类型

| 包类型 | 后缀 | 构建环境 | glibc 上限 | 适用场景 |
|--------|------|----------|------------|----------|
| native | `-native` | 宿主机直接 `cargo build` | 依赖构建机 glibc | 本机快速测试 |
| glibc2.28 | `-glibc2.28` | Rocky Linux 8 容器 | 2.28 | **企业兼容推荐包**，跨服务器部署验证 |
| musl | — | — | — | 暂不实现，未来如需极致跨发行版兼容再评估 |

关键设计：

- **glibc2.28 包**通过 `Dockerfile.release.glibc228`（基于 `rockylinux:8`）在容器内编译，确保二进制不引入 GLIBC_2.29+ 符号。
- 在新系统上编译后直接发送给老系统会因 glibc 符号版本不匹配而失败（例如本机 glibc 2.34 → 目标 RHEL 8 glibc 2.28 报 `GLIBC_2.32 not found`）。
- `scripts/package-release.sh [VERSION] [SUFFIX]` 支持后缀参数（`native` / `glibc2.28`），打包后自动输出 ldd 和 GLIBC 符号。glibc2.28 strict 模式同时检查 `lightai-server` 和 `lightai-agent`，任一出现 GLIBC_2.29+ 或 `libsqlite3.so` 则 exit 1。
- `scripts/package-release-docker.sh [VERSION]` 全自动流程：构建 Docker 镜像 → 容器内 `cargo build --release` → `cargo test --workspace` → `cargo clippy` → `npm run build` → `package-release.sh` → GLIBC 符号校验 → libsqlite3.so 依赖校验。
- SQLite 已 bundled，Web 已自托管，release 包不依赖 nginx。
- Docker/NVIDIA Driver/nvidia-container-toolkit 仍是 Docker/vLLM/GPU 测试时的外部环境依赖，不在 release 包内。
