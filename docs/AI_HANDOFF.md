...
- 当前交接时，分支相对 `origin/main` 是干净状态。
- Server 是 Rust 服务，提供健康检查、Agent 注册、Bearer token 心跳鉴权、节点/GPU 当前状态、历史指标、配置策略、运行环境、模型、模型文件、实例、模型垃圾箱、日志和审计能力。
...
- Agent 通过主动任务控制通道接收受控任务，包括模型文件验证、运行环境检查、本地实例启动/停止/测试、模型文件清理和 Agent 日志读取。
...
- 日志：Server、Agent、实例日志有基础读取入口；日志写入和读取会做敏感信息隐藏。
- 审计：Server 对配置、模型、运行环境、实例、模型垃圾箱等关键操作记录审计事件，Web 提供筛选查看。
...
1. 优先完善本地实例可靠性：服务就绪探测、端口冲突检查、日志 tail 刷新和异常退出状态。
2. 本地进程可靠性稳定后，再完善 Docker 运行模板。
3. 补强失败操作的审计覆盖。
4. 增加历史指标保留清理、基础聚合和降采样。
5. 本地运行层稳定后，再做 OpenAI-compatible Gateway、API Key、路由和用量统计。
6. 根据实际硬件需求增加厂商 GPU collector adapter。
7. 评估引入正式 migration ledger，减少代码里的 schema 修正逻辑。
