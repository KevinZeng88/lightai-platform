...
pub(super) fn validate_non_empty(field: &str, value: &str) -> Result<(), DomainError> {
...
pub(super) fn validate_backend(value: &str) -> Result<(), DomainError> {
    validate_one_of(
        "backend",
        value,
        &[
...
pub(super) fn validate_deploy_type(value: &str) -> Result<(), DomainError> {
    validate_one_of("deploy_type", value, &["docker", "script", "binary"])
}

pub(super) fn validate_model_type(value: &str) -> Result<(), DomainError> {
    validate_one_of(
        "model_type",
        value,
...
pub(super) fn validate_one_of(
    ...
    if allowed.contains(&value) {
        ...
        Err(DomainError::BadRequest(format!("{field} is invalid")))
    }
}
...
pub(super) fn validate_path(field: &str, value: &str) -> Result<(), DomainError> {
    if value.trim().is_empty() || value.contains("..") {
        return Err(DomainError::BadRequest(format!("{field} is invalid")));
    }
    Ok(())
}

pub(super) fn validate_no_whitespace(field: &str, value: &str) -> Result<(), DomainError> {
...
pub(super) fn validate_json_field(field: &str, value: Option<&str>) -> Result<(), DomainError> {
    if let Some(value) = value {
        serde_json::from_str::<serde_json::Value>(value).map_err(|_| {
            DomainError::BadRequest(format!("{field} must be valid JSON when provided"))
        })?;
    }
    Ok(())
}
...
pub(super) async fn ensure_model_exists(
    ...
    if exists.is_some() {
        ...
        Err(DomainError::NotFound("model not found".to_string()))
    }
}
...
pub(super) async fn ensure_node_exists(
    ...
    if exists.is_some() {
        ...
        Err(DomainError::NotFound("node not found".to_string()))
    }
}

pub(super) async fn node_online(pool: &SqlitePool, node_id: &str) -> Result<bool, DomainError> {
...
pub(super) fn map_sqlx_conflict(error: sqlx::Error) -> DomainError {
    match &error {
        ...
        {
            DomainError::Conflict("model name already exists; use a different name".to_string())
        }
        _ => DomainError::Internal(error.into()),
    }
}
