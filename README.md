...
轻量企业级 GPU / 模型服务管理平台。仓库是 Rust workspace + Vue/Vite Web monorepo，包含中央 Server、节点 Agent 和 Web 控制台。

项目当前处于第一阶段（v0.1），重点是轻量控制面、本地交付、多台 GPU 服务器统一纳管、基础模型实例管理、运行时检查、日志审计和控制台能力。后续方向是模型服务化、独立 Gateway 数据面、API Key、Usage、Quota、Cost、报表和治理能力。

当前 v0.1 不提供独立 Gateway 数据面、OpenAI-compatible API、API Key、请求级 Usage、配额或费用归集；这些属于后续规划能力。
...
| Runtime Environment | 某节点上的运行能力，包含 backend 和 deploy_type（v0.1 主流程为 `binary` / `docker`；`script` 仅保留后端兼容） |
...
| 后端 | 部署方式 | v0.1 状态 |
|------|---------|----------|
| **llama.cpp** | binary（本地进程） | 可用；gpu_layers 默认不传参，由运行环境自行决定 |
...
| **lmdeploy / mindie / Triton** | binary / docker | 预留；未完整验证 |
| **custom** | binary | 可用；`script` 后端兼容保留但不作为 v0.1 主流程推荐 |
...
- 本地实例主流程支持 `binary` / `docker` Runtime；Ollama 使用共享 daemon 模式。`script` 后端兼容保留，但 Web 不再作为 v0.1 新建推荐路径展示。
...
| 第二阶段 | 模型服务管理与独立 Gateway 数据面，包括 OpenAI-compatible API、模型路由和调用认证 | 未实现 |
...
- 第一阶段不实现独立 Gateway 数据面、OpenAI-compatible API、API Key 管理、额度、限流、调用统计、计量、费用归集、复杂报表和告警。
...
后续服务化方向应保持现有边界：模型服务调用入口规划为独立 Gateway 数据面，宜作为独立进程或独立二进制运行，并由 Agent 在节点侧托管生命周期；Server 保持控制面，负责 Service、API Key、用量统计、配额、成本核算、审计和策略管理，不默认承载业务模型流量；Web 继续只调用 Server API，不直连 Agent 或节点本地模型服务。
...
- 暂无 Kubernetes 集成
- 暂无 GPU 自动调度和扩缩容
- 暂无独立 Gateway 数据面和 OpenAI-compatible API
